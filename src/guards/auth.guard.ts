...
export class AuthGuard implements CanActivate {
    ...
    async canActivate(context: ExecutionContext) {
        ...
        try{         
            const data = this.authService.checkToken((authorization ?? '').split(' ')[1])//Pegando somente a parte 1 do array com o token, para aí sim retornar o payload, e fazendo o check do token. 
